...
function triggerRefValue(target, newValue, oldValue, isTrigger = true) {
    trigger(target, "set", "value", newValue, oldValue, isTrigger)
}
...
function handlerIterateDeps(vue2Observer, key, key2) {
    const iterateDeps = vue2Observer[key2] && vue2Observer[key2].deps
    if (iterateDeps) {
...
function trigger(target, type, key, newValue, oldValue, isTrigger = true) {
    if (reactiveProxyMap.has(target)) {
        const ctx = reactiveProxyMap.get(target)
        const vue2Observer = ctx.proxyObs
        const deps = [key]
        switch (type) {
            ...
            case "add": {
                let key2;
                if (!isArray(target)) {
                    key2 = isObject2(target) ? null : "size"
                } else {
                    if (isIntegerKey(key)) {
                        key2 = "length"
                    } else {
                ...
                if (isTrigger) {
                    ...
                    if (ITERATE_KEY in vue2Observer) {
                        handlerIterateDeps(vue2Observer, key, ITERATE_KEY)
                    }
                }
            }
                break
        ...
        if (deps.length) {
            ...
            for (let i = 0; i < deps.length; i++) {
                ...
                if (isTrigger && deps[i] === key) {
                    triggerOnTrigger(ctx.proxyTarget, vue2Observer[key], type, key, newValue, oldValue)
                }
                if (deleteFlag) {
...
function setVue2ObserverTargetValue(vue2Observer, key) {
    if (setVue2ObserverTargetReactive(vue2Observer, key)) {
        ...
        _index = (vue2Observer[key]._index = (observableValue.length >> _index) - 1);
...
function triggerReactive(ref2) {
    ...
    if (isProxy(ref2)) {
        ...
        if (isObject2(raw) || isArray(raw)) {
            ...
            if (isArray(raw)) {
                trigger(raw, "set", 'length', void 0, raw.length, false);
            }
        } else {
            for (let [w, v] of raw) {
                trigger(raw, "set", w, v, v);
            }
            trigger(raw, "set", 'size', void 0, raw.size, false);
        }
        trigger(raw, "set", ITERATE_KEY, void 0, true, false);
    }
}
...
export {
    ...
    watchEffect,
    watchPostEffect,
    watchSyncEffect, effectScope, effect, triggerRef, nextTick, triggerReactive
}
